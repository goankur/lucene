 ...
 * implements {@link ParallelVectorReadable}; all other files (HNSW graph, quantized codes,
 * metadata) are served unchanged by the wrapped {@link FSDirectory}, staying page-cached.
 *
...
public final class IoUringDirectory extends FilterDirectory {
   ...
   * closes.
   */
  private static final class UringInput extends FilterIndexInput implements ParallelVectorReadable {
    private final IoUring engine;
    ...
    public void readVectors(long[] positions, int dim, int count, float[] out) throws IOException {
      engine.read(fd, positions, dim, count, out);
    }
